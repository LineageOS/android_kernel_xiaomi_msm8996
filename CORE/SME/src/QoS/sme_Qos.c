...
/**
 * sme_QosTspecActive() - API exposed to HDD to check no of active Tspecs
 *
 ...
 *
 * Return: VOS_TRUE if success, otherwise VOS_FALSE
 */
                        ...
                        uint8_t sessionId, uint8_t *pActiveTspec)
{
  ...
  if (SME_QOS_QOS_ON == pACInfo->curr_state) {
     ...
     if (pACInfo->tspec_mask_status) {
         ...
         if (SME_QOS_TSPEC_MASK_BIT_1_2_SET == pACInfo->tspec_mask_status)
             *pActiveTspec = 2; /* both TSPECS are active */
         else
             *pActiveTspec = 1; /* only one TSPEC is active */
     } else {
        *pActiveTspec = 0;
     }
  } else {
