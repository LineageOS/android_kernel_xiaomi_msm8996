		...
		if (event == FB_EVENT_BLANK) {
			if (*transition == FB_BLANK_POWERDOWN) {
				...
				if (0 == fpc1020->wakeup_enabled) {
					// disable IRQ when screen turns off, only if wake_up disabled
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fpc1020->screen_on && fpc1020->proximity_state == 1) {
		// disable IRQ when screen is off and proximity sensor blocked
		if (fpc1020->irq_enabled) {
			spin_lock(&fpc1020->irq_lock);
			disable_irq(gpio_to_irq(fpc1020->irq_gpio));
			fpc1020->irq_enabled = false;
	...
	} else if (!fpc1020->screen_on && fpc1020->proximity_state == 0
			&& fpc1020->wakeup_enabled == 1) {
		// enable IRQ when screen is off, proximity sensor unblocked, BUT wake_up enabled
		if (!fpc1020->irq_enabled) {
			spin_lock(&fpc1020->irq_lock);
			enable_irq(gpio_to_irq(fpc1020->irq_gpio));
			fpc1020->irq_enabled = true;
