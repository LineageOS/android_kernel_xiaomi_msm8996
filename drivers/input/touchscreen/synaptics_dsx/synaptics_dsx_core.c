...
static void synaptics_secure_touch_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->st_initialized = 0;
	init_completion(&rmi4_data->st_powerdown);
	init_completion(&rmi4_data->st_irq_processed);
	/* Get clocks */
	rmi4_data->core_clk = clk_get(rmi4_data->pdev->dev.parent, "core_clk");
	if (IS_ERR(rmi4_data->core_clk)) {
		ret = PTR_ERR(rmi4_data->core_clk);
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: error on clk_get(core_clk):%d\n", __func__, ret);
		return;
	}

	rmi4_data->iface_clk = clk_get(rmi4_data->pdev->dev.parent, "iface_clk");
	if (IS_ERR(rmi4_data->iface_clk)) {
		ret = PTR_ERR(rmi4_data->iface_clk);
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: error on clk_get(iface_clk)\n", __func__);
		goto err_iface_clk;
	}

	rmi4_data->st_initialized = 1;
	return;

err_iface_clk:
		clk_put(rmi4_data->core_clk);
		rmi4_data->core_clk = NULL;
}
static void synaptics_secure_touch_notify(struct synaptics_rmi4_data *rmi4_data)
{
	sysfs_notify(&rmi4_data->pdev->dev.parent->kobj, NULL, "secure_touch");
}
static irqreturn_t synaptics_filter_interrupt(struct synaptics_rmi4_data *rmi4_data)
{
	...
	int blocking)
{
	if (atomic_read(&rmi4_data->st_enabled)) {
		...
		synaptics_secure_touch_notify(rmi4_data);
		if (blocking)
			wait_for_completion_interruptible(&rmi4_data->st_powerdown);
	}
}
#else
static void synaptics_secure_touch_init(struct synaptics_rmi4_data *rmi4_data)
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	struct i2c_client *client = container_of(rmi4_data->pdev->dev.parent,
	...
	case 0:
		...
		synaptics_secure_touch_clk_disable_unprepare(rmi4_data);
	...
	case 1:
		...
		if (synaptics_secure_touch_clk_prepare_enable(rmi4_data) < 0) {
			pm_runtime_put_sync(adapter);
			err = -EIO;
			break;
		}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->is_jdi_panel = rmi4_data->chip_id == CHIP_ID_3330 ? true : false;
