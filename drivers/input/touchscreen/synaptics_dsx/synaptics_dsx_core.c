...
static int synaptics_rmi4_proc_init(struct kernfs_node *sysfs_node_parent)
{
	...
	if (double_tap_sysfs_node)
		sprintf(double_tap_sysfs_node, "/sys%s/%s", buf, "wake_gesture");
	...
	if (key_disabler_sysfs_node)
		sprintf(key_disabler_sysfs_node, "/sys%s/%s", buf, "0dbutton");
	...
	if (swap_keys_sysfs_node)
		sprintf(swap_keys_sysfs_node, "/sys%s/%s", buf, "reversed_keys");
